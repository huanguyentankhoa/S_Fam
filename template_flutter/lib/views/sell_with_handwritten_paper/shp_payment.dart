...
import 'package:youreal/views/sell_with_handwritten_paper/shp_change_owner.dart';
class ShpPayment extends StatefulWidget {
  final Deal deal;
  const ShpPayment({Key? key, required this.deal}) : super(key: key);

  @override
  _ShpPaymentState createState() => _ShpPaymentState();
}

class _ShpPaymentState extends State<ShpPayment> {
  Widget _eventStreamBuild() {
    return Column(
      children: [
        timelineStartBuild("13/05", "Thanh toán toàn bộ"),
        ...
        child: GestureDetector(
          onTap: (){
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => ShpChangeOwner(deal: widget.deal,),
              ),
            );
          },
          child: Container(
