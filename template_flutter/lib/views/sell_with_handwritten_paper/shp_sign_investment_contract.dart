...
import 'package:youreal/views/sell_with_handwritten_paper/shp_payment.dart';
class ShpSignInvestmentContract extends StatefulWidget {
  final Deal deal;
  const ShpSignInvestmentContract({Key? key, required this.deal}) : super(key: key);

  @override
  _ShpSignInvestmentContractState createState() => _ShpSignInvestmentContractState();
}

class _ShpSignInvestmentContractState extends State<ShpSignInvestmentContract> {

  Widget _eventStreamBuild() {
    return Column(
      children: [
        timelineStartBuild("12/05", "Ký hợp đồng và đặt cọc"),
                  ...
                  InkWell(
                    onTap: () async {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => ShpPayment(deal: widget.deal,),
                        ),
                      );
                    },
                    child: Container(
